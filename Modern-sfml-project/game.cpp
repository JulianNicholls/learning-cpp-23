...
void Game::loop()
{
    ...
    while (window_.isOpen())
    {
        ...
        else
        {
            ...
            emanager_.apply<Ball>(
                [this](auto &ball)
                { emanager_.apply<Paddle>([&ball](auto &paddle) { handle_collision(paddle, ball); }); });

            emanager_.apply<Ball>(
                [this](auto &ball)
                { emanager_.apply<BashableBrick>([&ball](auto &brick) { handle_collision(ball, brick); }); });

            emanager_.apply<Ball>([this](auto &ball)
                                  { emanager_.apply<Brick>([&ball](auto &brick) { handle_collision(ball, brick); }); });
...
void Game::show_text(int frame_num)
{
    ...
    switch (state_)
    {
        case State::paused: str = "Paused"; break;

        case State::unstarted: str = "Press S to Start"; break;

        case State::player_won: str = "Player Wins"; break;

        case State::game_over: str = "Game Over"; break;
    ...
    auto opacity = cosf((frame_num % 64) * M_PI / 31.5f);
